...
def download_course(driver, course_code, base_dir, verbose=False):
    """Downloads the materials for a sungle course

    Args:
        driver : webdriver
        course_code (str): course code
        base_dir (str): path to the base download directory 
        verbose (bool, optional): Sets the Verbosity. Defaults to False.
    """
    try:
        driver.get(f"{MATERIALS_URL}resources/{YEAR}/{course_code}")
        course_name = driver.find_element_by_xpath(
            '//*[@id="main"]/div[2]/div/span[2]').text
        try:
            authenticate(driver)
        except:
            pass
        try:
            materials_list = driver.find_element_by_xpath(
                '//*[@id="Course Material"]/ul').find_elements_by_tag_name("li")

            for material in materials_list:
                if "\nFile" in material.text:
                    download_dir = f'{base_dir}/{course_code} - {course_name}/'
                    download(driver, download_dir)
                    material.find_element_by_tag_name(
                        "div").find_element_by_tag_name("a").click()
            print(f"Downloaded materials for {course_code} - {course_name}")
        except:
            if verbose:
                print(
                    f"No materials available for {course_code}{ f' - {course_name}' if course_name != 'None' else '' }")
    except NoSuchElementException:
        print("Something went wrong with your authentication.")
        print(
            f"Your shortcode is set as {SHORTCODE} and your password is set as {PASSWORD}")
...
def download_courses(driver, base_dir="./downloads/", verbose=False):
    """Uses download_course to download all the materials into a base directory

    Args:
        driver : webdriver
        base_dir (str, optional): Base Directory. Defaults to "./downloads/".
        verbose (bool, optional): Sets the verbosity. Defaults to False.
    """
    try:
        courses_list = driver.find_element_by_xpath(
            '//*[@id="main"]/div[3]/div/ul').find_elements_by_tag_name("li")
